sap.ui.define([
	...
	var oView, oSAPuser, t, company = "",
		arregloT = [];
	return Controller.extend("AR_DP_REP_FACTURACIONREPORTE_EXPO.AR_DP_REP_FACTURACIONREPORTE_EXPO.controller.master", {
		...
		onListItemPress: function (oEvent) {
			...
			sap.ui.controller("AR_DP_REP_FACTURACIONREPORTE_EXPO.AR_DP_REP_FACTURACIONREPORTE_EXPO.controller.detalle").jsoncreacion(json[productPath],
				arrtotal);
			var oRouter =
				sap.ui.core.UIComponent.getRouterFor(this);
		...
		EnvioCorreo: function (evt) {
			...
			if (!oDialog) {
				...
				oDialog = sap.ui.xmlfragment(oView.getId(), "AR_DP_REP_FACTURACIONREPORTE_EXPO.AR_DP_REP_FACTURACIONREPORTE_EXPO.view.Correo", this);
		...
		popSuccesCorreo: function (obj, titulo) {
			...
			if (!oDialog) {
				// create dialog via fragment factory
				oDialog = sap.ui.xmlfragment(oView.getId(), "AR_DP_REP_FACTURACIONREPORTE_EXPO.AR_DP_REP_FACTURACIONREPORTE_EXPO.view.SuccesCorreo", this); //aqui se debe cambiar ar_dp_rep
				oView.addDependent(oDialog);
			}
		...
		popCarga: function () {
			...
			if (!oDialog) {
				// create dialog via fragment factory
				oDialog = sap.ui.xmlfragment(oView.getId(), "AR_DP_REP_FACTURACIONREPORTE_EXPO.AR_DP_REP_FACTURACIONREPORTE_EXPO.view.PopUp", this);
				oView.addDependent(oDialog);
			}
			oDialog.open();
		...
		popSucces: function (obj, titulo) {
			...
			if (!oDialog) {
				// create dialog via fragment factory
				oDialog = sap.ui.xmlfragment(oView.getId(), "AR_DP_REP_FACTURACIONREPORTE_EXPO.AR_DP_REP_FACTURACIONREPORTE_EXPO.view.Succes", this);
				oView.addDependent(oDialog);
			}
